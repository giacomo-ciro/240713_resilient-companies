...
with col1:
    ...
    st.pyplot(fig)
    fig, ax = plt.subplots(figsize=(15, 5))
    industry_avg.plot(y = 'Growth', kind = 'bar', stacked=True, title='Industry Average Growth Before 2020', ax = ax)
    st.pyplot(fig)
...
with col2:
    ...
    st.pyplot(fig)
    fig, ax = plt.subplots(figsize=(15, 5))
    industry_avg.plot(y = 'Growth', kind = 'bar', stacked=True, title='Industry Average Growth After 2020', ax = ax)
    st.pyplot(fig)
    temp['OM_above'] = temp.apply(lambda x: is_above_industry_avg(x, 'OM', industry_avg), axis = 1)
    temp['Growth_above'] = temp.apply(lambda x: is_above_industry_avg(x, 'Growth', industry_avg), axis=1)
    temp = temp.groupby(['Name', 'Industry'])[['OM_above', 'Growth_above']].all()
